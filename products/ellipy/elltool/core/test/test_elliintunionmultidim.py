...
class TestElliIntUnionTCMultiDim:
    ...
    def test_is_internal(self):
        def test_correct(is_two_check, ans_vec):
            test_res_vec = Ellipsoid.is_internal(np.array([test_ell_array.flatten()]).T, test_point_vec, 'i')
            np.all(ans_vec == test_res_vec)
            if is_two_check:
                test_res_vec = Ellipsoid.is_internal(np.array([test_ell_array.flatten()]).T, test_point_vec, 'u')
                np.all(ans_vec == test_res_vec)
        ...
        def test_error(flag):
            ...
            if flag == 1 or flag == 2 or flag == 3 or flag == 8:
                test_ell_array_in, error_str = create_typical_array(flag)
            elif flag == 4:
                test_ell_array_in, _, error_str, _ = create_typical_array(flag)
        ...
        test_res_vec_out = Ellipsoid.is_internal(np.array([test_ell_array.flatten()]).T, test_point_vec, 'u')
...
def create_typical_array(flag):
    ...
    if flag == 1:
        ...
        test_ell_array = create_object_array(array_size_vec, ell_unitball, 4, 1, 1)
        test_ell_array[0, 0, 0, 0, 0, 0] = Ellipsoid(np.array([0, 0, 0, 1]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 0, 1] = Ellipsoid(np.array([0, 0, 0, -1]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 0, 2] = Ellipsoid(np.array([0, 0, 1, 0]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 1, 0] = Ellipsoid(np.array([0, 0, -1, 0]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 1, 1] = Ellipsoid(np.array([0, 1, 0, 0]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 1, 2] = Ellipsoid(np.array([0, -1, 0, 0]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 2, 0] = Ellipsoid(np.array([1, 0, 0, 0]).T, test_mat)
        test_ell_array[0, 0, 0, 0, 2, 1] = Ellipsoid(np.array([-1, 0, 0, 0]).T, test_mat)
        varargout1 = test_ell_array
...
def create_object_array(array_size_vec, func, first_arg, second_arg, n_arg):
    ...
    if n_arg == 1:
        object_c_array = np.empty((n_elems, 1), dtype=Ellipsoid)
        for i in range(0, int(n_elems)):
