    ...
    def test_is_internal(self):
        ...
        def test_error(flag):
            ...
            if flag == 1 or flag == 2 or flag == 3 or flag == 8:
                test_ell_array_in, error_str = _create_typical_array(self, flag)
            elif flag == 4:
                test_ell_array_in, _, error_str, _ = _create_typical_array(self, flag)
            elif flag == 5 or flag == 6 or flag == 7:
                test_ell_array_in, _, error_str = _create_typical_array(self, flag)
            with pytest.raises(Exception) as e:
                if test_ell_array_in.size > 0:
                    test_ell_array_in.flat[0].is_internal(test_ell_array_in, test_point_vec, 'u')
                else:
                    self.ellipsoid().is_internal(test_ell_array_in, test_point_vec, 'u')
            assert error_str in str(e.value)

        array_size_vec = np.array([2, 3, 2, 1, 1, 1, 4])
        test_ell_array = _create_object_array(array_size_vec, self.ell_unitball, 3, 1, 1)
        test_point_vec = 0.9 * np.eye(3)
        test_correct(True, np.array([1, 1, 1]))

        array_size_vec = np.array([1, 2, 2, 3, 1, 4])
        test_ell_array = _create_object_array(array_size_vec, self.ell_unitball, 2, 1, 1)
        test_point_vec = 1.1 * np.eye(2)
        test_correct(True, np.array([0, 0]))

        array_size_vec = np.array([1, 1, 1, 1, 1, 7, 1, 1, 7])
        test_ell_array = _create_object_array(array_size_vec, self.ell_unitball, 5, 1, 1)
        test_point_vec = 0.9 * np.eye(5)
        test_correct(True, np.array([1, 1, 1, 1, 1]))

        array_size_vec = np.array([2, 1, 2, 1, 3, 3])
        test_ell_array = _create_object_array(array_size_vec, self.ell_unitball, 4, 1, 1)
        ...
        test_ell_array, _ = _create_typical_array(self, 8)
...
def _create_typical_array(ell_factory_obj, flag):
    ...
    if flag == 1:
        ...
        test_ell_array = _create_object_array(array_size_vec, ell_unitball, 3, 1, 1)
        varargout1 = test_ell_array
        varargout2 = ell_unitball(3)
        return tuple((varargout1, varargout2))
    elif flag == 2:
        array_size_vec = np.array([1, 2, 4, 3, 2])
        test_ell_array = _create_object_array(array_size_vec, ell_unitball, 2, 1, 1)
        varargout1 = test_ell_array
        varargout2 = ell_unitball(2)
        return tuple((varargout1, varargout2))
    elif flag == 3:
        array_size_vec = np.array([1, 1, 1, 1, 1, 7, 1, 1, 7])
        test_ell_array = _create_object_array(array_size_vec, ell_unitball, 4, 1, 1)
        varargout1 = test_ell_array
        ...
        test2_ell_array = _create_object_array(array_size_vec, ell_unitball, 3, 1, 1)
...
def _create_object_array(array_size_vec, func, first_arg, second_arg, n_arg):
